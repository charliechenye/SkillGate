...
- Added the `skillgate` CLI with scan, check, diff, baseline, GitHub scan, inventory, policy, provenance, rules, explain, and fixture-summary commands.
...
- Implemented static rules `SG001` through `SG010` for shell execution, destructive commands, network egress, remote download execution, secret access, filesystem writes, prompt override language, Unicode obfuscation, MCP server configuration, and MCP capability drift.
...
- Added benchmark fixtures, expected finding summaries, and reduced public-pattern fixtures for Python, Node, shell, PowerShell, MCP, plugin hook, command-pack, local bridge, metadata, and agent-skill layouts.
